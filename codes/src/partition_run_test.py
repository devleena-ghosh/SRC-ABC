...
def get_next_time(frames, clauses, ttimes):
	...
	for ij in range(frames):
		...
		cla = clauses[ij]
		tt = ttimes[ij]
		if cla > 0 and (cla not in ctrain):
			ftrain.append(frm)
			ctrain.append(cla)
			ttrain.append(tt)
...
def get_reward(asrt, frames, clauses, mems, times, ttimes ):
	rewards = [[],[],[]]
	cu_rewards = [[],[],[]]
	cu_re1 = 0
	cu_re2 = 0
	...
	for j in range(len(frames)):
		...
		#reward 2
		nt, nd = get_next_time(frames[0:j], clauses[0:j], ttimes[0:j])
		cu_re2 += to/(1+frame)
...
def part_res(a, fname, ofname, TO, name, part):
	...
	rewards1, cu_rewards1 = get_reward(as1, frames1, clauses1, mems1, times1, ttimes1)
...
def main(argv):
	...
	filename = "plots/BMC_records_single_0808_{0}_{1}_{2}.csv".format(TIMEOUT, fname, len(Parts))
	...
	pdfname = "plots/plot_Partition_0808_all_{0}_{1}_{2}.pdf".format(fname, TIMEOUT, len(Parts))
	...
	for i in range(k):
		...
		if 2 in Parts:
			tr2, part2, rows2 = part_res(i, fname, ofname, To1, 'Partition-1', 2)
		...
		if 3 in Parts:
			tr3, part3, rows3 = part_res(i, fname, ofname, To2, 'Partition-2', 3)
		...
		if 4 in Parts:
			tr4, part4, rows4 = part_res(i, fname, ofname, To3, 'Partition-3', 4)
		...
		# plt.subplot(2, 2, 1)
		if 1 in Parts:
		    ax.plot(frames1, clauses1, 'b', label='total')
		if 2 in Parts:
			ax.plot(frames2, clauses2, 'r', label='Partition-1')
		if 3 in Parts:
			ax.plot(frames3, clauses3, 'g', label='Partition-2')
		if 4 in Parts:
			ax.plot(frames4, clauses4, 'c', label='Partition-3')
		...
		# plt.subplot(2, 2, 1)
		if 1 in Parts:
			ax.plot(frames1, mems1, 'b', label='total')
		if 2 in Parts:
			ax.plot(frames2, mems2, 'r', label='Partition-1')
		if 3 in Parts:
			ax.plot(frames3, mems3, 'g', label='Partition-2')
		if 4 in Parts:
			ax.plot(frames4, mems4, 'c', label='Partition-3')
		# plt.plot(frames4, clauses4, 'c', label='Part-3')
		# plt.legend(bbox_to_anchor=(1.3, 0.5))
		ax.set_xlabel("Depth")
		...
		if 1 in Parts:
			...
			ax.plot(ffms, ttms, 'b', label='total')
		if 2 in Parts:
			ffms, ttms = ttms_ffms(frames2, times2)
			ax.plot(ffms, ttms, 'r', label='Partition-1')
		if 3 in Parts:
			ffms, ttms = ttms_ffms(frames3, times3)
			ax.plot(ffms, ttms, 'g', label='Partition-2')
		if 4 in Parts:
			ffms, ttms = ttms_ffms(frames4, times4)
			ax.plot(ffms, ttms, 'c', label='Partition-3')
		# plt.legend(bbox_to_anchor=(1.3, 0.5))
		ax.set_xlabel("Depth")
		ax.set_ylabel("times")
		...
		if 1 in Parts:
			# ffms, ttms = ttms_ffms(frames1, times1)
			ax.plot(ttimes1, frames1, 'b', label='total')
		if 2 in Parts:
			# ffms, ttms = ttms_ffms(frames2, times2)
			ax.plot(ttimes2, frames2, 'r', label='Partition-1')
		if 3 in Parts:
			# ffms, ttms = ttms_ffms(frames3, times3)
			ax.plot(ttimes3, frames3, 'g', label='Partition-2')
		if 4 in Parts:
			# ffms, ttms = ttms_ffms(frames4, times4)
			ax.plot(ttimes4, frames4, 'c', label='Partition-3')
		# plt.legend(bbox_to_anchor=(1.3, 0.5))
		ax.set_xlabel("Depth")
		ax.set_ylabel("times")
		...
		plt.subplot(2, 2, 1)
		if 1 in Parts:
			plt.plot(frames1, rewards1[0], 'b', label='total')
		if 2 in Parts:
			plt.plot(frames2, rewards2[0], 'r', label='Partition-1')
		if 3 in Parts:
			plt.plot(frames3, rewards3[0], 'g', label='Partition-2')
		if 4 in Parts:
			plt.plot(frames4, rewards4[0], 'c', label='Partition-3')
		# plt.legend(bbox_to_anchor=(1.3, 0.5))
		plt.xlabel("Depth")
		plt.ylabel("Reward1")
		...
		plt.subplot(2, 2, 2)
		if 1 in Parts:
			plt.plot(frames1, rewards1[1], 'b', label='total')
		if 2 in Parts:
			plt.plot(frames2, rewards2[1], 'r', label='Partition-1')
		if 3 in Parts:
			plt.plot(frames3, rewards3[1], 'g', label='Partition-2')
		if 4 in Parts:
			plt.plot(frames4, rewards4[1], 'c', label='Partition-3')
		# plt.legend(bbox_to_anchor=(1.3, 0.5))
		plt.xlabel("Depth")
		plt.ylabel("Reward2")
		...
		plt.subplot(2, 2, 3)
		if 1 in Parts:
			plt.plot(frames1, rewards1[2], 'b', label='Partition')
		if 2 in Parts:
			plt.plot(frames2, rewards2[2], 'r', label='Partition-1')
		if 3 in Parts:
			plt.plot(frames3, rewards3[2], 'g', label='Partition-2')
		if 4 in Parts:
			plt.plot(frames4, rewards4[2], 'c', label='Partition-3')
		# plt.legend(bbox_to_anchor=(1.3, 0.5))
		plt.xlabel("Depth")
		plt.ylabel("Reward2")
		...
		plt.subplot(2, 2, 4)
		if 1 in Parts:
			plt.plot(frames1, clauses1, 'b', label='total')
		if 2 in Parts:
			plt.plot(frames2, clauses2, 'r', label='Partition-1')
		if 3 in Parts:
			plt.plot(frames3, clauses3, 'g', label='Partition-2')
		if 4 in Parts:
			plt.plot(frames4, clauses4, 'c', label='Partition-3')
		# plt.legend(bbox_to_anchor=(1.3, 0.5))
		plt.xlabel("Depth")
		plt.ylabel("Reward2")
		plt.title(Actions[i]+" -- Depth vs clauses")
